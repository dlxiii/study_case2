...
otfile_bash = ['../../csv_files_wa/multi_run_m.sh'];
info_file = ['../../csv_files_wa/info_month_layers.csv'];
...
for c = 1:length(caselist)
    for l = 1:length(layerlist)
        ...
        for m = 1:length(month)
            data1 = double(wa.(caselist{c}).(month{m})(:,layer));
...
otfile_bash = ['../../csv_files_wa/multi_run_a.sh'];
info_file = ['../../csv_files_wa/info_annual_layers.csv'];
fileID0 = fopen(otfile_bash,'w');
fileID1 = fopen(info_file,'w');
fprintf(fileID0,'#!/bin/bash\n');
...
% f = 1;m = 1;
for f = 1:length(caselist)
    wa.(caselist{f}).m00_va = 0;
    for m = 1:length(month)
        wa.(caselist{f}).m00_va = ...
            mean(wa.(caselist{f}).m00,2);
    end
end

otfile_bash = ['../../csv_files_wa/multi_run_ava.sh'];
info_file = ['../../csv_files_wa/info_annual_vertical.csv'];
fileID0 = fopen(otfile_bash,'w');
fileID1 = fopen(info_file,'w');
fprintf(fileID0,'#!/bin/bash\n');
%fprintf(fileID0,'# source /home/usr0/n70110d/usr/local/anaconda3/2019.03.py3/etc/profile.d/conda.sh\n');
...
otfile_bash = ['../../csv_files_ml_inner/multi_run_a.sh'];
info_file = ['../../csv_files_ml_inner/info_annual_layers.csv'];
fileID0 = fopen(otfile_bash,'w');
fileID1 = fopen(info_file,'w');
fprintf(fileID0,'#!/bin/bash\n');
fprintf(fileID0,'# source /home/usr0/n70110d/usr/local/anaconda3/2019.03.py3/etc/profile.d/conda.sh\n');
    ...
    for l = 1:length(layerlist)
        ...
        for m = 1:length(mmean)
            ...
            M = [reshape(X,[numel(X),1]),...
                reshape(Y,[numel(Y),1]),...
                reshape(data_temp.ageint,[numel(data_temp.ageint),1])];
            if exist('../../csv_files_ml_inner', 'dir')~=7
                mkdir('../../csv_files_ml_inner')
            end
            filename = [caselist{c},'_',mmean{m},'_',num2str(layer,formatSpec)];
            otfile_csv = ['../../csv_files_ml_inner/',...
                 filename,'.csv'];
